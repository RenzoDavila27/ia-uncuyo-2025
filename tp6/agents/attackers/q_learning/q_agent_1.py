    ...
    try:
        ...
        for episode in range(1, args.episodes + 1):
                if not early_stop:
                    ...
                    if args.testing:
                        agent._logger.info(f"Test results for episode {episode}:\n" + 
                                       f"Win rate: {eval_win_rate:.2f}%\n" +
                                       f"Detection rate: {eval_detection_rate:.2f}%\n" +
                                       f"Average returns: {eval_average_returns:.2f} ± {eval_std_returns:.2f}\n" +
                                       f"Average steps: {eval_average_episode_steps:.2f} ± {eval_std_episode_steps:.2f}")
                    ...
                    if episode % args.test_each == 0 and episode != 0:
                        ...
                        if not args.testing:
                            ...
                            agent._logger.info(f"Training evaluation after {episode} episodes:\n" +
                                           f"Win rate: {eval_win_rate:.2f}%\n" +
                                           f"Detection rate: {eval_detection_rate:.2f}%\n" +
                                           f"Average returns: {eval_average_returns:.2f} ± {eval_std_returns:.2f}\n" +
                                           f"Average steps: {eval_average_episode_steps:.2f} ± {eval_std_episode_steps:.2f}\n" +
                                           f"Current epsilon: {agent.current_epsilon}")
                            ...
                            for test_episode in range(1, args.test_for + 1):
                                ...
                                if episode % args.store_models_every == 0 and episode != 0:
                                    agent.store_q_table(f'q_agent_marl-episodes-{episode}.pickle')
                            ...
                            print(text)
                            # Log test results
                            agent._logger.info(f"Test results after {test_episode} test episodes:\n" +
                                           f"Win rate: {test_win_rate:.2f}%\n" +
                                           f"Detection rate: {test_detection_rate:.2f}%\n" +
                                           f"Average returns: {test_average_returns:.2f} ± {test_std_returns:.2f}\n" +
                                           f"Average steps: {test_average_episode_steps:.2f} ± {test_std_episode_steps:.2f}\n" +
                                           f"Current epsilon: {agent.current_epsilon}")
