...
class BaseAgent(ABC):
    ...
    def communicate(self, data:Action)-> tuple:
        ...
        def _receive_data(socket)->tuple:
            ...
            def _load_single_json(payload: str):
                """
                Parse a JSON payload, tolerating extra concatenated objects by using the first one.
                """
                try:
                    ...
                    try:
                        data_obj, index = decoder.raw_decode(payload)
                    except json.JSONDecodeError:
                        raise exc
                    remainder = payload[index:].strip()
                    if remainder:
                        preview = remainder[:200]
                        self._logger.warning(f"Extra data after JSON payload truncated to 200 chars: {preview}")
                    return data_obj

            # extract data from string representation
            data_dict = _load_single_json(data)
